...
def to_char_level_tags(words: List[str], named_entities: List[str]):
    result_words = list()
    result_named_entities = list()
    for word, named_entity in zip(words, named_entities):

        for char in word:
            result_words.append(char)
            result_named_entities.append(named_entity)

    l = len(result_named_entities)
    for idx in range(l - 1, 0, -1):
        last_tag = result_named_entities[idx - 1]
        this_tag = result_named_entities[idx]

        if this_tag.startswith('B') and this_tag == last_tag:
            result_named_entities[idx] = 'I{}'.format(this_tag[1:])

    return result_words, result_named_entities
    ...
    with open(to_filename, 'w', encoding='utf-8') as f:
        for conll_file in tqdm(ontonotes_reader.dataset_path_iterator(file_dir)):
            ...
            for sentence in ontonotes_reader.sentence_iterator(conll_file):
                char_words, char_pos_tags = to_char_level_tags(sentence.words, sentence.named_entities)
                sentence = ''.join(char_words)
