...
# nohup sh run.sh --system_version centos --stage -2 --stop_stage 5 &
...
trained_model_name=sentence_pair_bert
...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
  $verbose && echo "stage -1: download pretrained models"
  cd "${work_dir}" || exit 1;
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  python3 step_3_make_json_config.py \
  --pretrained_model_path "${pretrained_model_dir}" \
  --train_subset "${train_subset}" \
  --valid_subset "${valid_subset}" \
  --vocabulary_dir "${vocabulary_dir}" \
